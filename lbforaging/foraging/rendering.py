...
class Viewer(object):
    def __init__(self, world_size):
        ...
        self.img_food = pyglet.resource.image("battery_green.png")
        self.img_food_little = pyglet.resource.image("battery_pale_green.png")
        self.img_food_medium = pyglet.resource.image("battery_medium_green.png")
        self.img_food_full = pyglet.resource.image("battery_full_green.png")

        self.img_agent = [
            pyglet.resource.image("robot.png"),
            pyglet.resource.image("robot_circle.png")
        ]
        self.img_agent_evil = [
            pyglet.resource.image("robot_evil.png"),
            pyglet.resource.image("robot_evil_circle.png")
        ]
        self.img_agent_happy = [
            pyglet.resource.image("robot_happy.png"),
            pyglet.resource.image("robot_happy_circle.png")
        ]
    ...
    def _draw_food(self, env):
        ...
        for row, col in idxes:
            nutrition = env.field[row, col]
            if nutrition == 1:
                img = self.img_food_little
            elif nutrition == 2:
                img = self.img_food_medium
            else:
                img = self.img_food_full
            apples.append(
    ...
    def _draw_players(self, env):
        ...
        for i, player in enumerate(env.players):
            ...
            # represent fairness value
            if player.fairness < 0.4:
                img = self.img_agent_evil[i]
            elif player.fairness > 0.6:
                img = self.img_agent_happy[i]
            else:
                img = self.img_agent[i]

            # # represent score
            # if player.score <= 0.1:
            #     img = self.img_agent_evil
            # elif (player.score > 0.1) and (player.score <= 0.25):
            #     img = self.img_agent
            # else:
            #     img = self.img_agent_happy
